...
public class PoisonPortionMimic : EnemyCombatAI
{
    public GameObject poisonProjectilePrefab;
    public Color poisonColor = Color.green;
    public float poisonDuration = 3f;
    public float poisonTickDamageMultiplier = 0.1f;
    ...
    public override void Attack()
    {
        if (CheckTarget())
        {
            ...
            GameObject projectile = Instantiate(poisonProjectilePrefab, transform.position, Quaternion.identity);
            PoisonProjectile projectileComponent = projectile.GetComponent<PoisonProjectile>();
            if (projectileComponent != null)
            {
                projectileComponent.Initialize(cachedNearestTarget.transform, AttackDamage, this, poisonColor, poisonDuration, poisonTickDamageMultiplier);
            }
        }
